...
Preserve the architecture's verified-token API boundary. The F05/F06 implementation keeps refresh credentials in HTTP-only cookies through narrowly scoped web auth handlers, protects cookie-backed changes against CSRF, and keeps journal business rules in FastAPI. A thin web session bridge supplies short-lived access credentials held in memory for direct API calls. It adds no business proxy or localStorage session. Native clients will use platform-appropriate secure storage and the same API. See [authentication](../authentication.md) for implemented lifecycle and deployment limits.
